...
MenuDrinks::MenuDrinks(QWidget *parent, int idorder) :
    QWidget(parent), ui(new Ui::MenuDrinks), m_iddrink(0), m_idorder(idorder)
{
    // Call ui interface.
    ui->setupUi(this);
    ...
    m_xmlrpc = new XmlRpcClient(this, Loader::printDinerServerNameAndPort(),
                                "categoria-bebida", "getCategorias");
...
void MenuDrinks::readSettings(const QString &conf)
{
    QSettings settings(conf, QSettings::IniFormat);

    settings.beginGroup("MenuDrinks");

    // Read time random images.
    m_timerandomadsimages = settings.value("timeRandomDrinksImages").toInt();

    settings.endGroup();
}

void MenuDrinks::setIdOrder(int idorder)
{
    m_idorder = idorder;
}

int MenuDrinks::printIdCurrentOrder()
{
    return m_idorder;
}

void MenuDrinks::finishedDownload()
{
    // Set image.
    this->imageFrameRandom();
}

void MenuDrinks::showMyOrder()
{
    ListOrder *w = new ListOrder(0);

    w->startListDrinks(m_iddrink);
...
void MenuDrinks::showDetails()
{
    ListItemDetails *w = new ListItemDetails(0);

    w->startDetailsDrinks(m_iddrink);
...
void MenuDrinks::closeForms()
{
    this->disableObjects(false);
}

void MenuDrinks::disableObjects(bool disable)
{
    if(disable) {
        this->ui->frameMenuDrinksButtons->setDisabled(true);
        this->ui->listWidget->setDisabled(true);
    } else {
        this->ui->frameMenuDrinksButtons->setDisabled(false);
        this->ui->listWidget->setDisabled(false);
    }
}

void MenuDrinks::xmlRpcError()
{
...
void MenuDrinks::xmlRpcCallCategory()
{
    ...
    ui->frameMenuDrinksButtons->setLayout(layout);
}

void MenuDrinks::buttonMenuWasClicked(int buttonid)
{
    ui->listWidget->clear();
    ...
    for(int i = 0; i < m_buttonlist.size(); ++i) {
        ...
        if(m_buttonlist[buttonid]->objectName() == QString::number(id)) {
            const QString param = "bebida.cd_categoria = " + QString::number(id);
...
void MenuDrinks::xmlRpcCallDrinks()
{
    QList<QVariant> data = m_xmlrpc->printValues().toList();
    ...
    for(int i = 0; i < data.size(); ++i) {
        ...
        if(active == 1) {
           const int identify = data[i].toMap().value("cd_bebida").toInt();
           price = data[i].toMap().value("preco").toString();
           QString volume = data[i].toMap().value("volume").toString();
           const QString volumeRep = volume.replace(".00", "");
           ...
           QLabel *labelDrinkName = new QLabel(name);
           labelDrinkName->setStyleSheet("QLabel { background-color : transparent; "
                                        "color : white; font: bold 15px;}");

           QLabel *labelVolume = new QLabel(volumeRep + " " + volumeAC +
                                            "    ");
           labelVolume->setStyleSheet("QLabel { background-color : transparent; "
                                      "color : white; font: bold 15px;}");
           ...
           layout->addWidget(labelDrinkName);
...
void MenuDrinks::buttonListAddWasClicked(int buttonid)
{
    ui->listWidget->setCurrentRow(buttonid);
    showMyOrder();
}

void MenuDrinks::buttonListDetailWasClicked(int buttonid)
{
    ui->listWidget->setCurrentRow(buttonid);
    showDetails();
}

void MenuDrinks::imageFrameRandom()
{
    ...
    QStringList strlistfilter  = strlist.filter(QRegExp("^(images-dynamic/MenuDrinks)", Qt::CaseInsensitive));
...
void MenuDrinks::currentItemChanged(QListWidgetItem *current, QListWidgetItem *previous)
{
    if(!current) {
        current = previous;
    }
    ...
    if(!current->whatsThis().isEmpty() | !current->whatsThis().isNull()) {
        m_iddrink = current->whatsThis().toInt();
    }
}

void MenuDrinks::itemClicked(QListWidgetItem *item)
{
    ...
    if(!item->whatsThis().isEmpty() | !item->whatsThis().isNull()) {
        m_iddrink = item->whatsThis().toInt();
    }
}
